...
-- somewhere at the top of your module/script:

-- keep track of what *we* actually created
local createdInstances = {}

-- getOrCreate(className, name, parent)
--  • if parent[name] exists and :IsA(className), just returns it
--  • otherwise makes one, parents it, remembers it, and returns it
local function getOrCreate(className, name, parent)
    assert(typeof(name) == "string" and typeof(className) == "string",
           "Bad arguments to getOrCreate")
...
-- call this when you Destroy() your controller
-- to clean up only the things *you* made
local function cleanupCreated()
    ...
    createdInstances = {}  -- reset
end

--------------------------------------------------------------------------------
-- Example usage inside your GravityController or whatever:
...
function GravityController.new(player)
    ...
    -- you do:
    self.Camera = getOrCreate("Camera", "GravityCam", workspace)

    -- instead of
    --    local jumpModule = Instance.new("ModuleScript")
    -- you do:
    local jumpModule = getOrCreate("ModuleScript", "Jump", player.PlayerScripts)

    -- …and so on for any other helper instances…
    return self
end

function GravityController:Destroy()
    -- first do your normal tear-down
    -- e.g. unbind ContextActionService, disconnect events, etc.

    -- *then* destroy only the instances this module created:
    cleanupCreated()
